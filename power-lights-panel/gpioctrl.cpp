...
gpioctrl::gpioctrl(bool initWiringPi, bool wantSpi)
{
...
int gpioctrl::readRotation(int control)
{
    ...
    if (watcherThread) {
        if (rotateValue[control] != lastRotateValue[control]) {
...
int gpioctrl::readPush(int control)
{
    ...
    if (watcherThread) {
        if (pushValue[control] != lastPushValue[control]) {
